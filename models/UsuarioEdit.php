...
class UsuarioEdit extends Usuario
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("ID_USUARIO") ?? Key(0) ?? Route(2)) !== null) {
                $this->ID_USUARIO->setQueryStringValue($keyValue);
                $this->ID_USUARIO->setOldValue($this->ID_USUARIO->QueryStringValue);
            } elseif (Post("ID_USUARIO") !== null) {
                $this->ID_USUARIO->setFormValue(Post("ID_USUARIO"));
                $this->ID_USUARIO->setOldValue($this->ID_USUARIO->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'CONTRASENA' first before field var 'x_CONTRASENA'
        $val = $CurrentForm->hasValue("CONTRASENA") ? $CurrentForm->getValue("CONTRASENA") : $CurrentForm->getValue("x_CONTRASENA");
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            $this->CONTRASENA->EditValue = HtmlEncode($this->CONTRASENA->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        if ($this->ID_USUARIO->Required) {
            if (!$this->ID_USUARIO->IsDetailKey && EmptyValue($this->ID_USUARIO->FormValue)) {
                $this->ID_USUARIO->addErrorMessage(str_replace("%s", $this->ID_USUARIO->caption(), $this->ID_USUARIO->RequiredErrorMessage));
            }
        }
        ...
        if ($this->CONTRASENA->Required) {
            if (!$this->CONTRASENA->IsDetailKey && EmptyValue($this->CONTRASENA->FormValue)) {
    ...
    protected function editRow()
    {
        ...
        // CONTRASENA
        $this->CONTRASENA->setDbValueDef($rsnew, $this->CONTRASENA->CurrentValue, null, $this->CONTRASENA->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['CONTRASENA'] == $this->CONTRASENA->CurrentValue);
