...
class EquipotorneoList extends Equipotorneo
{
    ...
    // CSS class/style
    public $CurrentPageName = "equipotorneolist";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if ($pageName != $this->getListUrl()) { // Not List page => View page
                    ...
                    $result["view"] = $pageName == "equipotorneoview";
                } else { // List page
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "fequipotorneogrid";
        }
    ...
    public function showFilterList()
    {
        ...
        // Field ID_EQUIPO_TORNEO
        $filter = $this->queryBuilderWhere("ID_EQUIPO_TORNEO");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ID_EQUIPO_TORNEO, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ID_EQUIPO_TORNEO->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        // Field ID_EQUIPO
        $filter = $this->queryBuilderWhere("ID_EQUIPO");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->ID_EQUIPO, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->ID_EQUIPO->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field PARTIDOS_JUGADOS
        $filter = $this->queryBuilderWhere("PARTIDOS_JUGADOS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->PARTIDOS_JUGADOS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->PARTIDOS_JUGADOS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field PARTIDOS_GANADOS
        $filter = $this->queryBuilderWhere("PARTIDOS_GANADOS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->PARTIDOS_GANADOS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->PARTIDOS_GANADOS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field PARTIDOS_EMPATADOS
        $filter = $this->queryBuilderWhere("PARTIDOS_EMPATADOS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->PARTIDOS_EMPATADOS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->PARTIDOS_EMPATADOS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field PARTIDOS_PERDIDOS
        $filter = $this->queryBuilderWhere("PARTIDOS_PERDIDOS");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->PARTIDOS_PERDIDOS, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->PARTIDOS_PERDIDOS->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field GF
        $filter = $this->queryBuilderWhere("GF");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->GF, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->GF->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field GC
        $filter = $this->queryBuilderWhere("GC");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->GC, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->GC->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field GD
        $filter = $this->queryBuilderWhere("GD");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->GD, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->GD->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field GRUPO
        $filter = $this->queryBuilderWhere("GRUPO");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->GRUPO, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->GRUPO->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field POSICION_EQUIPO_TORENO
        $filter = $this->queryBuilderWhere("POSICION_EQUIPO_TORENO");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->POSICION_EQUIPO_TORENO, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->POSICION_EQUIPO_TORENO->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }
        ...
        if ($this->isInlineEditRow()) { // Inline-Edit
            ...
            $cancelurl = $this->addMasterUrl($pageUrl . "action=cancel");
                ...
                if ($this->UseAjaxActions) {
                    ...
                    $opt->Body = <<<INLINEEDIT
                    <div{$divClass}>
                        <button class="ew-grid-link ew-inline-update" title="{$updateTitle}" data-caption="{$updateTitle}" form="fequipotorneolist" formaction="{$inlineUpdateUrl}">{$updateCaption}</button>
            ...
            $opt->Body .= "<input type=\"hidden\" name=\"" . $this->OldKeyName . "\" id=\"" . $this->OldKeyName . "\" value=\"" . HtmlEncode($this->ID_EQUIPO_TORNEO->CurrentValue) . "\">";
            return;
        }
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"equipotorneo\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"equipotorneo\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"equipotorneo\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"equipotorneo\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        $item->Body = "<button type=\"button\" class=\"ew-action ew-multi-delete\" title=\"" .
            ...
            HtmlTitle($Language->phrase("DeleteSelectedLink")) . "\" form=\"fequipotorneolist\"" .
    ...
    public function setupRow()
    {
        ...
        if (($this->isGridAdd() || $this->isGridEdit())) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_equipotorneo", "data-rowtype" => ROWTYPE_ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_equipotorneo",
    ...
    {
        $this->PARTIDOS_JUGADOS->DefaultValue = $this->PARTIDOS_JUGADOS->getDefault(); // PHP
        $this->PARTIDOS_JUGADOS->OldValue = $this->PARTIDOS_JUGADOS->DefaultValue;
        $this->PARTIDOS_GANADOS->DefaultValue = $this->PARTIDOS_GANADOS->getDefault(); // PHP
        $this->PARTIDOS_GANADOS->OldValue = $this->PARTIDOS_GANADOS->DefaultValue;
        $this->PARTIDOS_EMPATADOS->DefaultValue = $this->PARTIDOS_EMPATADOS->getDefault(); // PHP
        $this->PARTIDOS_EMPATADOS->OldValue = $this->PARTIDOS_EMPATADOS->DefaultValue;
        $this->PARTIDOS_PERDIDOS->DefaultValue = $this->PARTIDOS_PERDIDOS->getDefault(); // PHP
        $this->PARTIDOS_PERDIDOS->OldValue = $this->PARTIDOS_PERDIDOS->DefaultValue;
        $this->GF->DefaultValue = $this->GF->getDefault(); // PHP
        $this->GF->OldValue = $this->GF->DefaultValue;
        $this->GC->DefaultValue = $this->GC->getDefault(); // PHP
        $this->GC->OldValue = $this->GC->DefaultValue;
        $this->GD->DefaultValue = $this->GD->getDefault(); // PHP
        ...
        if ($this->RowType == ROWTYPE_VIEW) {
            ...
            if ($this->ID_EQUIPO->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter("`ID_EQUIPO`", "=", $this->ID_EQUIPO->CurrentValue, DATATYPE_NUMBER, "");
                }
