...
class Register extends Usuario
{
    ...
    public function run()
    {
        ...
        if ($this->isInsert()) {
            ...
            $filter = GetUserFilter(Config("LOGIN_USERNAME_FIELD_NAME"), $this->USER->CurrentValue);
    ...
    {
        $this->nombre->DefaultValue = "torneo";
    }
    ...
    protected function loadFormValues()
    {
        ...
        if (Config("ENCRYPTED_PASSWORD")) { // Encrypted password, use raw value
            $this->CONTRASENA->ConfirmValue = $CurrentForm->getValue("c_CONTRASENA");
        } else {
            $this->CONTRASENA->ConfirmValue = RemoveXss($CurrentForm->getValue("c_CONTRASENA"));
        }
    ...
    protected function validateForm()
    {
        ...
        if ($this->USER->Required) {
            if (!$this->USER->IsDetailKey && EmptyValue($this->USER->FormValue)) {
                $this->USER->addErrorMessage($Language->phrase("EnterUserName"));
            }
        }
        if ($this->CONTRASENA->Required) {
            if (!$this->CONTRASENA->IsDetailKey && EmptyValue($this->CONTRASENA->FormValue)) {
                $this->CONTRASENA->addErrorMessage($Language->phrase("EnterPassword"));
            }
        }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // CONTRASENA
        $this->CONTRASENA->setDbValueDef($rsnew, $this->CONTRASENA->CurrentValue, null, false);
